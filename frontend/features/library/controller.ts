...
function classifyGamepadId(rawId: string): ControllerDeviceType {
	...
	if (id.includes('dualsense') || id.includes('ps5') || id.includes('0ce6') || id.includes('0df2')) {
		return 'dualsense';
	}
	if (id.includes('dualshock') || id.includes('ps4') || id.includes('05c4') || id.includes('09cc')) {
		return 'dualshock';
	}
...
function classifySteamControllerType(eType: number, name: string): ControllerDeviceType {
	...
	if (eType === 31 || eType === 32 || eType === 46) return 'xbox';
